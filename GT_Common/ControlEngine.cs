            ...
            var specificHandlers = new Dictionary<string, Func<ProcessContext, Task>>
            {
                [ProcessStageKeys.AfterDataRead] = ctx => {
                    ...
                    if (ctx.PartInfos == null || ctx.Data == null)
                    {
                        ...
                        DisplayLog.Warn($"流程 【{barBindProcess.Name}】【{ctx.Sn}】 数据异常NULL：PartInfos={ctx.PartInfos}, Data={ctx.Data}", true);
                    ...
                    if (ctx.PartInfos.Count != ctx.Data.Count)
                    {
                        ...
                        DisplayLog.Warn($"流程 【{barBindProcess.Name}】【{ctx.Sn}】 绑定数量不匹配：PartInfos={ctx.PartInfos.Count}, Data={ctx.Data.Count}", true);

                        throw new InvalidOperationException(
                            $"流程 【{barBindProcess.Name}】【{ctx.Sn}】 绑定数量不匹配：PartInfos={ctx.PartInfos.Count}, Data={ctx.Data.Count}");
                    }
            ...
            var specificHandlers = new Dictionary<string, Func<ProcessContext, Task>>
            {
                [ProcessStageKeys.AfterSnRead] = ctx => {
                    ...
                    if (!string.IsNullOrEmpty(ctx.Sn))
                    {
                        ...
                        string NgCode = UploadSql.SelectNgCode(ctx.Sn);
                        Regex regex = new Regex(@"^(\d+)_.*$");
                        Match match = regex.Match(NgCode);
                        if (match.Success)
                        {
                            ctx.WritePlcResult = int.Parse(match.Groups[1].Value);
                        }
                        else
                        {
                            ctx.WritePlcResult = 2;
            ...
            var specificHandlers = new Dictionary<string, Func<ProcessContext, Task>>
            {
                [ProcessStageKeys.AfterSnRead] = async ctx =>
                {
                    if (!string.IsNullOrEmpty(ctx.Sn))
                    {
                        ...
                        {
                            var rule = ProductCodeConfig.GetRule(Shared.productModel.BaseInfo.ProductName, CodeType.Shell);
                            ...
                            else
                            {
                                try
                                {
                                    var shopOrder = mesServer.ValidateOrderInfor("", Config.Instance.Resource, "", out bool result);
                                    if (!result)
                                    {
                                        ctx.WritePlcResult = 2;  // 无效订单
                                    ...
                                    ctx.MesOrder = shopOrder.Order;
                                    Shared.shopOrder = shopOrder;
                                    monitor.OnProductInput(ctx.Sn, shopOrder.Order, isRework: false);
                                ...
                                {
                                    ctx.WritePlcResult = 2;
                                    DisplayLog.Error($"流程 【{checkResultProcess.Name}】【{ctx.Sn}】 MES报错", ex, true);
                                }
                            }
                        }
                    }
